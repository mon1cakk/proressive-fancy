...
import { EVENTTYPES, HTTPTYPE, EMethods } from "@lesliejs/common";
import { ReplaceHandler, voidFun } from "@lesliejs/types";
...
function xhrReplace(): void {
  ...
  replaceAop(originalXhrProto, "send", (originalSend: voidFun) => {
    return function (this: any, ...args: any[]): void {
      const { method, url } = this.lesliejs_xhr;
      ...
      on(this, "loadend", function (this: any) {
        ...
        this.lesliejs_xhr.requestData = args[0];
        const eTime = getTimestamp();
        // 设置该接口的time，用户用户行为按时间排序
        this.lesliejs_xhr.time = this.lesliejs_xhr.sTime;
        this.lesliejs_xhr.Status = status;
        if (["", "json", "text"].indexOf(responseType) !== -1) {
          ...
          if (
            ...
            this.lesliejs_xhr.response = response && JSON.parse(response);
          }
        }
        // 接口的执行时长
        this.lesliejs_xhr.elapsedTime = eTime - this.lesliejs_xhr.sTime;
        // 执行之前注册的xhr回调函数
        triggerHandlers(EVENTTYPES.XHR, this.lesliejs_xhr);
      });
