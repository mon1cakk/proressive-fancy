...
import { SDK_VERSION, EVENTTYPES } from "@lesliejs/common";
import { ReportData, InitOptions } from "@lesliejs/types";
...
export class TransportData {
  ...
  getAuthId(): string | number {
    if (typeof this.getUserId === "function") {
      ...
      if (typeof id === "string" || typeof id === "number") {
        ...
        console.error(
          `lesliejs userId: ${id} 期望 string 或 number 类型，但是传入 ${typeof id}`
        );
      }
    }
    return "";
  ...
  bindOptions(options: InitOptions): void {
    ...
    validateOption(getUserId, "getUserId", "function") &&
  ...
  async send(data: ReportData) {
    ...
    if (isEmpty(dsn)) {
      console.error(
        "lesliejs: dsn为空，没有传入监控错误上报的dsn地址，请在init中传入"
      );
      return;
    }
    // 开启录屏
